...
/* ================================
   TAB 3 - RENDER NAV 14 MÔN PHÁI
================================ */

function renderTab3Nav() {
  const nav = document.getElementById("tab3Nav");
  if (!nav) return;

  let html = "";

  for (let i = 0; i < sectKeys.length; i++) {
    const key = sectKeys[i];
    const sect = sectData[key];
    const activeClass = i === 0 ? " is-active" : "";

    html += `
      <button class="tab3__sect-btn${activeClass}" type="button" data-sect="${key}">
        <span class="tab3__sect-icon">
          <img src="public/tab3/${sect.icon}" alt="">
        </span>

        <span class="tab3__sect-active-bg">
          <img src="public/tab3/act.webp" alt="">
        </span>
...
function initTab3Sect() {
  ...
  const heroBox = document.getElementById("tab3Hero");
